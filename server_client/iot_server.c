...
int main(int argc, char *argv[])
{
		...
		CLIENT_INFO client_info[MAX_CLNT] = {{0,-1,"","KMJA_SQL","PASSWD"}, \
				{0,-1,"","KMJA_UBT","PASSWD"},  {0,-1,"","KMJA_ARD","PASSWD"}, \
				{0,-1,"","KMJA_AND","PASSWD"},  {0,-1,"","KMJA_LIN","PASSWD"}, \
				{0,-1,"","KMJA_BT","PASSWD"},  {0,-1,"","7","PASSWD"}, \
...
void *clnt_connection(void *arg)
{
    ...
    while (1) {
        ...
        // LYH_ARD에서 받은 메시지는 모든 클라이언트에게 전송
...
void send_msg(MSG_INFO * msg_info, CLIENT_INFO * first_client_info)
{
    ...
    if (!strcmp(msg_info->from, "KMJA_ARD")) {
